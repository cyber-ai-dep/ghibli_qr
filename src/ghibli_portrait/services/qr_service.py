...
def get_qr(url: str, version: int = None):
    s = Settings()
    ...
    qr.make(fit=True)
    qr_img = qr.make_image(fill_color=s.QR_FILL_COLOR, back_color=s.QR_BACK_COLOR).convert('RGBA')

    qr_img = qr_img.resize((s.QR_SIDE_LEN, s.QR_SIDE_LEN), Image.LANCZOS)
    lock_img.paste(qr_img, s.QR_PASTE_COORDINATES, qr_img)

    lock_img.save('google_qr.png')



if __name__ == '__main__':
    get_qr('http://google.com')